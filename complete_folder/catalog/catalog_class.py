...
class Catalog(RoomCatalog):
    def __init__(self,myCatalog,catalogFileName):
            ...
            RoomCatalog.__init__(self,self.myCatalog,catalogFileName)
            self.catalogFileName=catalogFileName
            self.addressIP=myCatalog['catalog'][0].get('addressIP')
            self.port=myCatalog['catalog'][0].get('port')
            self.inactiveTime=myCatalog['inactiveTime']
            self.clientID=myCatalog['clientID']
            self.lastUpdate=myCatalog['last_update']

    #EX1.1
    def pingBot(self,botAddress):
        addressFinal=self.addressIP+':'+str(self.port)
        putBody={"catalogID":self.clientID,"addressIP":addressFinal}
        try:
            r=requests.put(botAddress+'/bot/insertCatalog', json=putBody)
        except:
    ...
    def insertRoom(self,roomID,devices=[]):
        notExisting=1
        now=datetime.now()
        timestamp=now.strftime("%d/%m/%Y %H:%M")
        ...
        if notExisting!=0:
            #newDevice=Device(sensorID,endPoints,availableResources,timestamp)
            devices=[]
            newRoom=RoomObj(roomID,roomID,devices,timestamp).jsonify()
