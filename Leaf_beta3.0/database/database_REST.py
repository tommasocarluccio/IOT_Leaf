...
class DatabaseManager(object):
    ...
    def PUT (self, *uri, ** params):
        ...
        if command=='insertCatalog':
            ...
            if newCatalog==True:
                ...
                output="'{}' already exists!".format(newCatalogID)
    ...
    def DELETE(self, *uri, ** params):
        command=str(uri[0])
        catalogID=str(uri[1])
        self.actualCatalog=self.server.retrieveCatalogID()
        self.c=CatalogManager(self.actualCatalog)
        if command== 'removeDevice':
            roomID=str(params['room'])
            deviceID=str(params['device'])
            self.actualRoom=self.c.roomsCatalog(roomID)
            self.d=DevicesCatalog(self.actualRoom)
