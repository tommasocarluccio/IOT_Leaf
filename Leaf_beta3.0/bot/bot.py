...
class bot_Class(object):
    ...
    def on_callback_query(self, msg):
        ...
        for i in self.room_list:
            room_ID=i['roomID']
            if query_data==self.id_name_dic[room_ID] and self.enter_room_flag==1: #respond to enter a room
                self.entered_room=self.id_name_dic[room_ID]
                self.bot.sendMessage(self.chat_id, f'Now you are in {self.entered_room}\nSelect an option:', reply_markup=self.room_menu)
                self.entered_room_flag=1
                self.enter_room_flag=0
            elif query_data==self.id_name_dic[room_ID] and self.delete_room_flag==1: #respond to remove a room
                selected_room=self.id_name_dic[room_ID]
        ...
        if room_list==[]:
            ...
            for room in room_list:
                ...
                try:
                    ...
                    last_update='None'
                try:
                    emo_room=self.emoji_dic[room_ID]
                except:
                    emo_room=':house:'
                        ...
                        if parameter=='temperature':
                            ...
                            if str(temp_int).isdecimal()==True:
                                if temp_int>=18 and temp_int<=30:
                                    self.flag_t=0
                                elif temp_int<18 and self.flag_t==0:
                                    time.sleep(3)
                                    try:
                                        self.bot.sendMessage(self.chat_id, emoji.emojize(f":cold_face: Internal temperature in {room_ID} too low!", use_aliases=True), reply_markup=self.find_more)
                                        self.flag_t=1
                                    except:
                                        pass
                                    ...
                                    time.sleep(3)
                                    try:
                                        self.bot.sendMessage(self.chat_id, emoji.emojize(f":hot_face: Internal temperature in {room_ID} too high!", use_aliases=True), reply_markup=self.find_more)
                                        self.flag_t=1
                                    except:
                                        pass
                        #CHECK ON HUMIDITY VALUE
                            ...
                            if str(hum_int).isdecimal()==True:
                                if hum_int>=30 and hum_int<=55:
                                    bot_c.flag_h=0
                                elif hum_int<18 and self.flag_h==0:
                                    time.sleep(3)
                                    try:
                                        self.bot.sendMessage(self.chat_id, emoji.emojize(f":cactus: Internal humidity in {room_ID} too low!", use_aliases=True), reply_markup=self.find_more)
                                        self.flag_h=1
                                    except:
                                        pass
                                    ...
                                    time.sleep(3)
                                    try:
                                        self.bot.sendMessage(self.chat_id, emoji.emojize(f":sweat_drops: Internal humidity in {room_ID} too high!", use_aliases=True), reply_markup=self.find_more)
                                        self.flag_h=1
                                    except:
                                        pass
                        #CHECK ON AQI VALUE
                            ...
                            if int(aqi_int)<750:
                                self.flag_a==0
                            elif int(aqi_int)>=750 and self.flag_a==0:
                                time.sleep(3)
                                try:
                                    self.bot.sendMessage(self.chat_id, emoji.emojize(f":red_circle: Internal AQI in {room_ID} too high!", use_aliases=True), reply_markup=self.find_more)
                                    self.flag_a=1
                                except:
                                    pass
